...
export class DownloadWorker {
    ...
    hasLeadingZeroNibbles(hash, difficulty) {
        let fullBytes = Math.floor(difficulty / 2);
        for (let i = 0; i < fullBytes; i += 1) {
            if (hash[i] !== 0)
                return false;
        }
        if (difficulty % 2 === 1 && (hash[fullBytes] & 0xf0) !== 0)
            return false;
        return true;
    }
    async deriveAltChallenge(challenge, nonce, solution, encoder) {
        const hash = await crypto.subtle.digest("SHA-256", encoder.encode(`pow:alt:${challenge}:${nonce}:${solution}`));
    ...
    async solvePow(challengeType, challenge, nonce, difficulty) {
        if (!challenge || !nonce || !difficulty)
            return "";
        const encoder = new TextEncoder();
        const first = await this.solveSinglePow(challengeType || "classic", challenge, nonce, difficulty, encoder);
        if (!first)
            return "";
    ...
    async loadBootstrapFromPage(context) {
        ...
        try {
            ...
            await page.waitForFunction(() => {
                ...
                timeout: Math.min(5000, this.config.timeoutSec * 1000)
            }).catch(() => { });
            ...
            const bootstrap = await page.evaluate(() => {
                ...
                return {
                    ...
                    powChallenge: String(data.powChallenge || "").trim(),
                    challengeType: String(data.challengeType || "classic").trim(),
                    powDifficulty: Number(data.powDifficulty || 0) || 0,
    ...
    async autolinkAuthFlow(context, targetUrl, skipHome = false) {
        const runHttp = async () => {
            const bj = await this.loadBootstrapFromPage(context);
            if (!bj)
                return { status: 500, phase: "bootstrap", text: "window_bootstrap_missing" };
            const solution = await this.solvePow(String(bj.challengeType || "classic"), String(bj.powChallenge || ""), String(bj.nonce || ""), Number(bj.powDifficulty || 0));
            const issueHeaders = {
                "X-Page-Nonce": bj.nonce,
                Origin: BASE,
                Referer: bj.pageUrl || Endpoints.home(),
                "User-Agent": this.config.userAgent,
                Accept: "application/json, text/plain, */*"
            };
            if (solution)
